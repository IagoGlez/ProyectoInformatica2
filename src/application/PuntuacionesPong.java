...
public class PuntuacionesPong {
public static void display() {
    	
    	
    	Stage window;
    	window = new Stage();
        window.setTitle("Puntuaciones Pong.");
		...
		try {
			 ...
			 rs = stmt.executeQuery("SELECT Puntuaciones.Id_Usuario, Puntuacion, Usuarios.Nombre FROM Puntuaciones LEFT JOIN Usuarios On Puntuaciones.Id_Usuario == Usuarios.ID_Usuario where Id_juego = 3"); //consulta (select, udpate, delete, insert)
			 while(rs.next()) { //Recorrer el resultado de la query 
